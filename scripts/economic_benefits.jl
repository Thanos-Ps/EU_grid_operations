...
result = simulate_case!(input_data, nodal_data, solver, Tmax, T0, prediction_horizon, number_of_clusters, repetitions, cable_id)

# Select no cables with DCR to simulate the reference case (DCR OFF)
cable_id = []

# Run the reference case
result_ref = simulate_case!(input_data_raw, nodal_data, solver, Tmax, T0, prediction_horizon, number_of_clusters, repetitions, cable_id)
