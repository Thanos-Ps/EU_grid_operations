...
elseif results_flag == "cable_results"
    ...
    for j in 1:number_of_clusters
        ...
        for i in repetitions[j]
            ...
            for hour = i : 1 : i + prediction_horizon - 1
                ...
                global temperature_sum += cable_data["$selected_cable"]["$hour"]["temperature"]["$(number_of_iterations[j,reps])"]
    ...
    percentage_above_100 = (count_above_100 / simulated_hours) * 100
    percentage_equal_100 = (count_equal_100 / simulated_hours) * 100
    percentage_below_100 = (count_below_100 / simulated_hours) * 100

    # Calculate the mean temperature
    global mean_temperature = temperature_sum / simulated_hours
    global mean_P_inj = P_inj_sum / simulated_hours
...
elseif results_flag == "injection_results"
    ...
    for j in 1:number_of_clusters
        ...
        for i in repetitions[j]
            global reps += 1
            for hour = i : 1 : i + prediction_horizon - 1
                println(cable_data["$selected_cable"]["$hour"]["pt"]["$(number_of_iterations[j,reps])"])
            end
        end
    end
