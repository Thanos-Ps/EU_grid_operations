...
examined_ratios = [1, 1.5, 2]
examined_cable_ratings = [10, 15, 20]
...
for cable_rating in examined_cable_ratings
    i[1] += 1
    j[1] = 0

    for ratio in examined_ratios
        j[1] += 1

        # Set grid parameters of each case
        local cable_capacity = cable_rating
        local converter_capacity = ratio*cable_capacity

        # Update cable_id and converter_id capacities
        ...
        # Reset cable_id and converter_id capacities
        for cable in cable_id
            input_data["branch"]["$cable"]["rate_a"] = cable_capacity
            input_data["branch"]["$cable"]["rate_p"] = cable_capacity
            input_data["branch"]["$cable"]["rate_i"] = cable_capacity
...
plot(examined_ratios, economic_benefit[:,1], xlabel=" Converter - Cable capacity ratio", ylabel = "Average Economic Benefit per Hour [%]", 
label = "$(examined_cable_ratings[1]/10) GW cable",size=(800, 600), linewidth=2, xlabelfontsize=14,   # Bigger x-axis label font
...
#=
for k in 2:length(examined_cable_ratings)

    plot!(examined_ratios, economic_benefit[:,k], label = "$(examined_cable_ratings[k]/10) GW cable", linewidth = 2)
   
end
=#

plot!(examined_ratios, economic_benefit[:,2], label = "$(examined_cable_ratings[2]/10) GW cable", linewidth = 2)

plot!(examined_ratios, economic_benefit[:,3], label = "$(examined_cable_ratings[3]/10) GW cable", linewidth = 2)
