...
if Flag1 == "over_hours"
    ...
    # Select over which time cluster results will be presented to ensure continuity over the temporal axis
    cluster = 1

    # Create a time vector for the x-axis (optional)
    time_vals = collect(t[cluster][1]:1:t[cluster][end])
    
    # Loop over all hours for the same iteration

    for i in repetitions[cluster]
        global reps += 1
        for hour = i : 1 : i + prediction_horizon - 1
            ...
            temperature_evolution = cable_data["$cable_id"]["$hour"]["temperature"]["$(number_of_iterations[cluster,reps])"]
    ...
    p1 = plot(time_vals, power_values, xlabel="Hour", ylabel="Rating [%]", title="Loading of Cable $cable_id at Time Slice $cluster", legend = false)
    p2 = plot(time_vals, temperature_values, xlabel="Hour", ylabel="Temperature [degC]", title="Temperature of Cable $cable_id at Time Slice $cluster", legend = false)
...
elseif Flag1 == "over_iterations"
        ...
        # Identify over which time cluster the selected hour belong

        for j in 1:length(t)
            if hour >= t[j][1] && hour <= t[j][end]
                global cluster = j
                break
            end
        end
    ...
    for j in 1:length(repetitions[cluster])
        if repetitions[cluster][j] > hour
            global reps = copy(j) - 1
            break

        else
            reps = length(repetitions[cluster])
        end
    end

    iteration_vector = 1:number_of_iterations[cluster,reps]
    
    # Loop over all iterations for the same hour

    for iteration in 1:number_of_iterations[cluster,reps]
